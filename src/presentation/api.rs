...
pub fn dispatch(app: &Arc<App>, method: &str, request_json: &str) -> String {
    ...
    let response = match method {
        ...
        "wan.set" | "wan.set_config" => {
            serde_json::from_value::<crate::domain::SetWanRequest>(request)
                .map_err(|error| {
                    DomainError::new(
                        ErrorCode::InvalidArgument,
                        ErrorStage::Validate,
        ...
        "tools.ping" => serde_json::from_value::<crate::application::tools::PingRequest>(request)
            .map_err(|error| {
                DomainError::new(
                    ErrorCode::InvalidArgument,
                    ErrorStage::Validate,
...
mod tests {
    ...
    use crate::{App, Device, MemoryBackend, StateStore};
    ...
    fn test_api_dispatch_devices_list() {
        ...
        let response_str = crate::presentation::api::dispatch(&app, "devices.list", "{}");
